...
func _ready():
	StarDustNet.subscribe_to_packet(self, InstanceData.TYPE_CREATE_INSTANCE)
	StarDustNet.subscribe_to_packet(self, InstanceData.TYPE_REMOVE_INSTANCE)
...
func create_net_instance(res_path:String, tree_path:String):
	if(StarDustNet.is_server()):
		...
		var np = NetPacket.new(InstanceData.TYPE_CREATE_INSTANCE, inst_dat.get_as_data())
...
func remove_net_instance(instance_id:int):
	if(StarDustNet.is_server()):
		if(_instance_nodes.has(instance_id)):
			...
			var np = NetPacket.new(InstanceData.TYPE_REMOVE_INSTANCE, inst_dat.get_as_data())
			StarDustNet.send_packet_reliable(np)
...
func sync_instances(player_id:int):
	if(StarDustNet.is_server()):
		for i in _instance_data:
			...
			var np:NetPacket = NetPacket.new(InstanceData.TYPE_CREATE_INSTANCE, inst.get_as_data(), player_id)
			StarDustNet.send_packet_reliable(np)

func packet_received(packet:NetPacket):
	if(!StarDustNet.is_server()):
		if(packet.type == InstanceData.TYPE_CREATE_INSTANCE):
			var inst_dat = InstanceData.map_data(packet.data)
			create_local_instance(inst_dat.instance_id, inst_dat.scene_resource_path, inst_dat.scene_tree_path)
		if(packet.type == InstanceData.TYPE_REMOVE_INSTANCE):
			var inst_dat = InstanceData.map_data(packet.data)
